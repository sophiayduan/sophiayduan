## hi i'm sophia 🧌

i like to build hardware projects and websites

most of my projects are here, and a few on my (very kicad themed) [`website`](https://sophiaduan.dev/)

#### below are my recent public contributions


- [`tachometer`](https://github.com/sophiayduan/tachometer) - _"A ESP32 based tachometer with a dashboard"_ **(3 days ago)**
- [`dotfiles`](https://github.com/sophiayduan/dotfiles) - _"my fedora rice"_ **(4 days ago)**
- [`website`](https://github.com/sophiayduan/website) - _"my personal kicad inspired website"_ **(2 weeks ago)**
- [`bldc-motor`](https://github.com/sophiayduan/bldc-motor) - _"a simple, mostly 3d printed bldc"_ **(3 weeks ago)**
- [`storyboard`](https://github.com/hackclub/storyboard) - _"Make a visual novel a month depending on the theme!"_ **(3 weeks ago)**




