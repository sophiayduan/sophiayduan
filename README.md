## hi i'm sophia 🧌

i like to build hardware projects and websites

most of my projects are here, and a few on my (very kicad themed) [`website`](https://sophiaduan.dev/)

#### below are my recent public contributions


- [`storyboard`](https://github.com/hackclub/storyboard) - _"Make a visual novel a month depending on the theme!"_ **(1 day ago)**
- [`fire-project`](https://github.com/sophiayduan/fire-project) - _""_ **(3 days ago)**
- [`tachometer`](https://github.com/sophiayduan/tachometer) - _"A ESP32 based tachometer with a dashboard"_ **(6 days ago)**
- [`dotfiles`](https://github.com/sophiayduan/dotfiles) - _"my fedora rice"_ **(1 week ago)**
- [`website`](https://github.com/sophiayduan/website) - _"my personal kicad inspired website"_ **(2 weeks ago)**




